...
class RiskBacktester:
    ...
    def run_comprehensive_backtest(self, df: pd.DataFrame, predictions_df: pd.DataFrame = None,
                                 start_date: str = '2019-01-01', end_date: str = '2023-12-31') -> Dict[str, Any]:
        """
        ...
        """
        self.logger.info(f"Running comprehensive backtest from {start_date} to {end_date}")
        ...
        results['event_analysis'] = self._analyze_market_events(backtest_data, predictions_df)
        
        # 2. Calculate prediction accuracy metrics
        if predictions_df is not None:
            results['prediction_accuracy'] = self._calculate_prediction_accuracy(backtest_data, predictions_df)
        
        # 3. Evaluate early warning capabilities
        results['early_warning_performance'] = self._evaluate_early_warning(backtest_data)
        
        # 4. Simulate portfolio performance using risk signals
        results['portfolio_simulation'] = self._simulate_portfolio_performance(backtest_data)
    ...
    def _prepare_backtest_data(self, df: pd.DataFrame, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        if 'Close' in filtered_df.columns and 'Symbol' in filtered_df.columns:
            ...
            filtered_df['Forward_Return_1d'] = filtered_df.groupby('Symbol')['Close'].pct_change().shift(-1)
            filtered_df['Forward_Return_5d'] = filtered_df.groupby('Symbol')['Close'].pct_change(5).shift(-5)
            filtered_df['Forward_Return_20d'] = filtered_df.groupby('Symbol')['Close'].pct_change(20).shift(-20)
    ...
    def _get_data_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'missing_data_pct': (df.isnull().sum().sum() / df.size * 100) if not df.empty else 0
        }
    ...
    def _analyze_market_events(self, df: pd.DataFrame, predictions_df: pd.DataFrame = None) -> Dict[str, Any]:
        ...
        for event_name, event_info in self.market_events.items():
            ...
            if event_data.empty:
                event_results[event_name] = {'status': 'no_data', 'description': event_info['description']}
                continue
            ...
            sentiment_cols = [col for col in event_data.columns if 'sentiment' in col.lower()]
            if sentiment_cols:
                ...
                for col in sentiment_cols[:3]:  # Top 3 sentiment columns
                    ...
                    if not sentiment_values.empty:
                        event_analysis['sentiment_analysis'][col] = {
                            ...
                            'negative_sentiment_pct': float((sentiment_values < 0).mean())
                        }
            ...
            if 'Risk_Label' in event_data.columns:
                ...
                event_analysis['risk_signals'] = {
                    ...
                    'avg_risk_score': float(event_data.get('Risk_Score', pd.Series([0])).mean())
                }
            
            # Early warning performance (did we predict this event?)
            pre_event_days = 30
            pre_event_start = event_start - timedelta(days=pre_event_days)
            pre_event_mask = (df['Date'] >= pre_event_start) & (df['Date'] < event_start)
            ...
            if not pre_event_data.empty and 'Risk_Label' in pre_event_data.columns:
                ...
                baseline_risk_rate = df['Risk_Label'].mean() if 'Risk_Label' in df.columns else 0
                
                # Early warning score: how much higher was risk before the event
                early_warning_score = (pre_event_risk_rate - baseline_risk_rate) / (baseline_risk_rate + 1e-6)
                event_analysis['early_warning_score'] = float(early_warning_score)
            
            # Model prediction analysis (if available)
            if predictions_df is not None and not predictions_df.empty:
                if 'Date' in predictions_df.columns and 'Risk_Probability' in predictions_df.columns:
                    pred_event_mask = (predictions_df['Date'] >= event_start) & (predictions_df['Date'] <= event_end)
                    pred_event_data = predictions_df[pred_event_mask]
                    
                    if not pred_event_data.empty:
                        event_analysis['prediction_performance'] = {
                            'avg_risk_probability': float(pred_event_data['Risk_Probability'].mean()),
                            'max_risk_probability': float(pred_event_data['Risk_Probability'].max()),
                            'high_risk_predictions_pct': float((pred_event_data['Risk_Probability'] > 0.7).mean())
                        }
    ...
    def _calculate_prediction_accuracy(self, df: pd.DataFrame, predictions_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if all(col in df.columns for col in ['Symbol', 'Date', 'Risk_Label']) and \
           all(col in predictions_df.columns for col in ['Symbol', 'Date', 'Risk_Probability']):
    ...
    def _evaluate_early_warning(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for symbol in df['Symbol'].unique():
            ...
            for idx in symbol_data[risk_event_starts].index:
                ...
                lookback_data = symbol_data[symbol_data.index < idx].tail(30)  # 30 days lookback
        ...
        if 'Risk_Score' in df.columns:
            ...
            warning_periods = df[warning_signals].groupby([df['Symbol'], signal_groups]).size()
    ...
    def _simulate_portfolio_performance(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'Risk_Score' in df.columns:
            ...
            daily_returns['Position_Size'] = 1.0 - (daily_returns['Risk_Score'] * 0.8)  # Reduce exposure when risk is high
            daily_returns['Position_Size'] = daily_returns['Position_Size'].clip(0.2, 1.0)  # Min 20% exposure
            
            # Calculate strategy returns
            daily_returns['Strategy_Return'] = daily_returns['Returns'] * daily_returns['Position_Size']
            daily_returns['Benchmark_Return'] = daily_returns['Returns']  # Buy and hold
            
            # Cumulative returns
            daily_returns['Strategy_Cumulative'] = (1 + daily_returns['Strategy_Return']).cumprod()
            daily_returns['Benchmark_Cumulative'] = (1 + daily_returns['Benchmark_Return']).cumprod()
            
            # Performance metrics
            total_periods = len(daily_returns)
            if total_periods > 0:
                strategy_total_return = daily_returns['Strategy_Cumulative'].iloc[-1] - 1
                benchmark_total_return = daily_returns['Benchmark_Cumulative'].iloc[-1] - 1
                
                strategy_volatility = daily_returns['Strategy_Return'].std() * np.sqrt(252)
                benchmark_volatility = daily_returns['Benchmark_Return'].std() * np.sqrt(252)
                
                # Sharpe ratio (assuming 0% risk-free rate)
                strategy_sharpe = (daily_returns['Strategy_Return'].mean() * 252) / strategy_volatility if strategy_volatility > 0 else 0
                benchmark_sharpe = (daily_returns['Benchmark_Return'].mean() * 252) / benchmark_volatility if benchmark_volatility > 0 else 0
                
                # Maximum drawdown
                strategy_peak = daily_returns['Strategy_Cumulative'].expanding().max()
                strategy_drawdown = (daily_returns['Strategy_Cumulative'] - strategy_peak) / strategy_peak
    ...
    def _calculate_risk_metrics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'Risk_Score' in df.columns and 'Date' in df.columns:
            ...
            df_sorted['Risk_Score_MA'] = df_sorted['Risk_Score'].rolling(rolling_window).mean()
            df_sorted['Risk_Score_Vol'] = df_sorted['Risk_Score'].rolling(rolling_window).std()
            ...
            risk_metrics['model_stability'] = {
                ...
                'model_drift': float(df_sorted['Risk_Score_MA'].iloc[-1] - df_sorted['Risk_Score_MA'].iloc[rolling_window])
            }
    ...
    def _create_html_backtest_report(self) -> str:
        ...
        for event_name, event_data in event_analysis.items():
            if isinstance(event_data, dict) and event_data.get('status') != 'no_data':
                ...
                if risk_signals:
                    ...
                    performance_class = 'performance-good' if risk_rate > 0.3 else 'performance-poor'
                ...
                warning_class = 'performance-good' if early_warning > 0.5 else 'performance-poor'
        ...
        if pred_accuracy:
            ...
            if auc_score is not None:
                auc_class = 'performance-good' if auc_score > 0.7 else 'performance-poor'
                html_content += f"<p><strong>AUC Score:</strong> <span class='{auc_class}'>{auc_score:.3f}</span></p>"
        
        # Portfolio Performance
        portfolio_perf = results.get('portfolio_simulation', {}).get('strategy_performance', {})
        if portfolio_perf:
            html_content += f"""
