...
class SECFilingsDownloader:
    ...
    def get_company_filings(self, cik: str, form_types: List[str] = ['10-K', '10-Q']) -> List[Dict]:
        """
        ...
        try:
            ...
            if response.status_code != 200:
                self.logger.error(f"Failed to get filings for CIK {cik}: {response.status_code}")
                return []
            ...
            for i, form in enumerate(forms):
                if form in form_types and i < len(accession_numbers) and i < len(filing_dates):
                    filings.append({
                        'cik': cik,
                        'form_type': form,
    ...
    def extract_sections(self, filing_content: str, sections: List[str] = None) -> Dict[str, str]:
        """
                ...
                for line in lines:
                    ...
                    for section in sections:
                        if section.lower() in line.lower() and len(line) < 200:
                            ...
                            current_section = section.lower().replace("'", "").replace(" ", "_")
                            section_content = []
                            break
                    ...
                    if current_section and line:
                        ...
                        if any(stop_word in line.lower() for stop_word in 
                               ['item 2', 'item 3', 'item 4', 'controls and procedures']):
                            extracted[current_section] = '\n'.join(section_content)
                            current_section = None
                            section_content = []
    ...
    def fetch_filings_for_symbols(self, symbols: List[str], max_filings_per_symbol: int = 5) -> pd.DataFrame:
        """
        ...
        for symbol in symbols:
            try:
                ...
                for filing in filings[:max_filings_per_symbol]:
                    self.logger.info(f"Processing {filing['form_type']} for {symbol} dated {filing['filing_date']}")
                    ...
                    if content:
                        ...
                        record = {
                            ...
                            'risk_factors_length': len(sections.get('risk_factors', '')),
        ...
        self.logger.info(f"Processed filings for {len(df['symbol'].unique())} companies")
