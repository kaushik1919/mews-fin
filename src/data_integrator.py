...
class DataIntegrator:
    ...
    def integrate_all_data(self, stock_df: pd.DataFrame, news_df: pd.DataFrame = None,
                          sec_df: pd.DataFrame = None, sentiment_df: pd.DataFrame = None) -> pd.DataFrame:
        """
        ...
        self.logger.info(f"Data integration completed: {len(integrated_df)} rows, {len(integrated_df.columns)} columns")
    ...
    def _merge_sentiment_data(self, stock_df: pd.DataFrame, sentiment_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'date' in sentiment_clean.columns:
            ...
            if hasattr(stock_df['Date'].dt, 'tz'):
                ...
                if stock_tz is None and sentiment_tz is not None:
                    ...
                    sentiment_clean['Date'] = sentiment_clean['Date'].dt.tz_localize(None)
                elif stock_tz is not None and sentiment_tz is None:
                    # Stock is timezone-aware, sentiment is naive -> localize sentiment
                    sentiment_clean['Date'] = sentiment_clean['Date'].dt.tz_localize('UTC').dt.tz_convert(stock_tz)
                elif stock_tz is not None and sentiment_tz is not None and stock_tz != sentiment_tz:
                    # Both timezone-aware but different -> convert sentiment to stock timezone
                    sentiment_clean['Date'] = sentiment_clean['Date'].dt.tz_convert(stock_tz)
            
            sentiment_clean.drop('date', axis=1, inplace=True)
        
        # Merge on Symbol and Date
        merge_cols = ['Symbol', 'Date']
        available_merge_cols = [col for col in merge_cols if col in stock_df.columns and col in sentiment_clean.columns]
        
        if available_merge_cols:
            merged_df = stock_df.merge(sentiment_clean, on=available_merge_cols, how='left')
            
            # Forward fill sentiment data (sentiment persists for a few days)
            sentiment_cols = [col for col in sentiment_clean.columns if col not in available_merge_cols]
            for col in sentiment_cols:
                if col in merged_df.columns:
                    merged_df[col] = merged_df.groupby('Symbol')[col].ffill()
            
            self.logger.info(f"Merged sentiment data: added {len(sentiment_cols)} sentiment features")
            return merged_df
        
        return stock_df
    
    def _merge_news_features(self, stock_df: pd.DataFrame, news_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(col in news_clean.columns for col in ['Symbol', 'Date']):
            news_agg = news_clean.groupby(['Symbol', 'Date']).agg({
            ...
            news_agg.columns = [
                'Symbol', 'Date', 'news_sentiment_mean', 'news_sentiment_std', 'news_count',
                'news_sentiment_intensity', 'news_title_length_avg', 'news_desc_length_avg'
            ]
            ...
            news_cols = ['news_sentiment_mean', 'news_sentiment_std', 'news_sentiment_intensity']
            for col in news_cols:
                merged_df[col] = merged_df.groupby('Symbol')[col].ffill()
            
            # Fill remaining NaN with neutral values
            merged_df['news_count'] = merged_df['news_count'].fillna(0)
            merged_df['news_sentiment_mean'] = merged_df['news_sentiment_mean'].fillna(0)
            merged_df['news_sentiment_std'] = merged_df['news_sentiment_std'].fillna(0)
            merged_df['news_sentiment_intensity'] = merged_df['news_sentiment_intensity'].fillna(0)
    ...
    def _merge_sec_features(self, stock_df: pd.DataFrame, sec_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(col in sec_clean.columns for col in ['Symbol', 'Date']):
            ...
            sec_features = [
                'Symbol', 'Date', 'mda_sentiment', 'risk_factors_sentiment',
                'combined_sec_sentiment', 'mda_word_count', 'risk_factors_word_count'
            ]
            existing_features = [col for col in sec_features if col in sec_clean.columns]
    ...
    def _add_derived_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in ['news_sentiment_mean', 'Returns']):
            df['sentiment_return_correlation'] = (
                df.groupby('Symbol')
                .apply(lambda x: x['news_sentiment_mean'].rolling(20).corr(x['Returns']))
        ...
        if all(col in df.columns for col in ['news_count', 'Volatility_20d']):
            df['news_volatility_ratio'] = df['news_count'] / (df['Volatility_20d'] + 1e-6)
        ...
        if all(col in df.columns for col in ['PE_Ratio', 'news_sentiment_mean']):
            ...
            df['pe_sentiment_risk'] = (df['PE_Ratio'] > df['PE_Ratio'].median()) & (df['news_sentiment_mean'] < -0.1)
        ...
        if all(col in df.columns for col in ['mda_sentiment', 'news_sentiment_mean']):
            df['sec_news_sentiment_diff'] = abs(df['mda_sentiment'] - df['news_sentiment_mean'])
            df['sentiment_alignment'] = df['sec_news_sentiment_diff'] < 0.2  # Aligned if difference < 0.2
        ...
        if 'Market_Cap' in df.columns:
            ...
            df['market_cap_decile'] = pd.qcut(df['Market_Cap'], 10, labels=False, duplicates='drop')
    ...
    def _add_lag_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        lag_features = [
            'Returns', 'Volatility_20d', 'news_sentiment_mean', 'news_count',
            'Volume_Ratio', 'RSI', 'PE_Ratio'
        ]
    ...
    def _add_market_regime_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['bull_market'] = (df['Market_Return'] > df['market_return_ma200']).astype(int)
        
        # Volatility regime
        if 'Market_Volatility' in df.columns:
            vol_median = df['Market_Volatility'].median()
            df['high_volatility_regime'] = (df['Market_Volatility'] > vol_median * 1.5).astype(int)
        ...
        if 'news_count' in df.columns:
            high_news = df.groupby('Date')['news_count'].transform('sum') > df.groupby('Date')['news_count'].transform('sum').quantile(0.95)
    ...
    def _clean_integrated_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        fill_zero_cols = [col for col in df.columns if any(keyword in col.lower() for keyword in 
                         ['sentiment', 'news', 'count', 'ratio', 'volume'])]
    ...
    def create_feature_groups(self, df: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        for col in df.columns:
            ...
            if any(keyword in col_lower for keyword in ['close', 'open', 'high', 'low', 'price', 'return']):
            ...
            elif any(keyword in col_lower for keyword in ['rsi', 'macd', 'ma_', 'bb_', 'volatility']):
                feature_groups['technical_indicators'].append(col)
            elif any(keyword in col_lower for keyword in ['pe_ratio', 'roe', 'debt', 'beta', 'pb_ratio']):
                feature_groups['fundamental_ratios'].append(col)
            elif any(keyword in col_lower for keyword in ['sentiment']) and 'news' not in col_lower:
            ...
            elif any(keyword in col_lower for keyword in ['mda', 'sec', 'risk_factors']):
            ...
            elif any(keyword in col_lower for keyword in ['divergence', 'alignment', 'regime', 'crisis']):
                feature_groups['derived_features'].append(col)
            elif any(keyword in col_lower for keyword in ['year', 'month', 'quarter', 'day', 'week']):
    ...
    def save_integrated_data(self, df: pd.DataFrame, feature_groups: Dict[str, List[str]], output_dir: str):
        ...
        with open(summary_path, 'w') as f:
            ...
            f.write(f"Unique symbols: {df['Symbol'].nunique() if 'Symbol' in df.columns else 'N/A'}\n")
