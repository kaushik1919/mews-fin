...
class SentimentAnalyzer:
    ...
    def _initialize_models(self):
        ...
        try:
            ...
            self.logger.warning("Transformers library not available - FinBERT sentiment disabled")
    ...
    def analyze_news_sentiment(self, news_df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['sentiment_label'] = df['combined_sentiment_compound'].apply(self._categorize_sentiment)
    ...
    def analyze_sec_sentiment(self, sec_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'mda_text' in df.columns and self.finbert_model is not None:
            ...
            df = self._analyze_vader_fallback(df, 'mda_text', 'mda_sentiment', 'mda_sentiment_label')
        ...
        if 'risk_factors_text' in df.columns and self.finbert_model is not None:
            ...
            risk_sentiments, risk_labels = self._analyze_finbert_sentiment(df['risk_factors_text'])
            ...
            # Fallback to VADER if FinBERT not available
            self.logger.info("Using VADER for Risk Factors sentiment (FinBERT not available)")
            df = self._analyze_vader_fallback(df, 'risk_factors_text', 'risk_factors_sentiment', 'risk_factors_sentiment_label')
    ...
    def _analyze_finbert_sentiment(self, texts: pd.Series) -> Tuple[List[float], List[str]]:
    ...
    def _analyze_vader_fallback(self, df: pd.DataFrame, text_col: str, 
                               sentiment_col: str, label_col: str) -> pd.DataFrame:
    ...
    def _add_sentiment_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in ['title_sentiment_compound', 'description_sentiment_compound']):
        ...
        if all(col in df.columns for col in ['published_date', 'symbol', 'combined_sentiment_compound']):
            df['date'] = df['published_date'].dt.date
            
            # Daily average sentiment by symbol
            daily_sentiment = df.groupby(['symbol', 'date'])['combined_sentiment_compound'].mean().reset_index()
    ...
    def _add_sec_sentiment_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if existing_cols:
            ...
            df['combined_sec_sentiment_label'] = df['combined_sec_sentiment'].apply(self._categorize_sentiment)
        
        # Risk sentiment change over time
        if all(col in df.columns for col in ['symbol', 'filing_date', 'risk_factors_sentiment']):
            df = df.sort_values(['symbol', 'filing_date'])
            df['risk_sentiment_change'] = (
                df.groupby('symbol')['risk_factors_sentiment'].diff()
            )
    ...
    def aggregate_sentiment_by_date(self, news_df: pd.DataFrame, 
                                   sec_df: pd.DataFrame = None) -> pd.DataFrame:
        """
        ...
        if news_df is not None and not news_df.empty and 'published_date' in news_df.columns:
            news_df['date'] = news_df['published_date'].dt.date
            
            news_agg = news_df.groupby(['symbol', 'date']).agg({
            ...
            news_agg.columns = [
                ...
                'news_sentiment_intensity', 'news_title_sentiment', 'news_description_sentiment'
            ]
        ...
        if sec_df is not None and not sec_df.empty and 'filing_date' in sec_df.columns:
            ...
            sec_agg = sec_df.groupby(['symbol', 'date']).agg({
                'mda_sentiment': 'mean',
            ...
            sec_agg.columns = [
                'symbol', 'date', 'sec_mda_sentiment', 'sec_risk_sentiment', 'sec_combined_sentiment'
            ]
        ...
        self.logger.info(f"Created sentiment aggregations for {len(result_df)} symbol-date combinations")
        return result_df
    ...
    def calculate_sentiment_indicators(self, sentiment_df: pd.DataFrame) -> pd.DataFrame:
        ...
        sentiment_cols = [col for col in df.columns if 'sentiment' in col and col.endswith('_mean')]
        ...
        for col in sentiment_cols:
            if col in df.columns:
                ...
                df[f'{base_name}_trend'] = df[f'{base_name}_ma7'] - df[f'{base_name}_ma30']
        ...
        if 'news_count' in df.columns:
            ...
            df['high_news_volume'] = (
                df['news_count'] > df.groupby('symbol')['news_count'].transform('mean') * 2
            ).astype(int)
        
        return df
    
    def save_sentiment_data(self, news_df: pd.DataFrame, sec_df: pd.DataFrame, 
                           aggregated_df: pd.DataFrame, output_dir: str):
