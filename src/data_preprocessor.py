...
class DataPreprocessor:
    ...
    def preprocess_stock_data(self, stock_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'Returns' in df.columns:
            ...
            for window in [5, 10, 20, 30]:
                df[f'Volatility_{window}d'] = (
                    ...
                    .transform(lambda x: x.rolling(window=window).std() * np.sqrt(252))
                )
        ...
        self.logger.info(f"Processed stock data: {len(df)} rows, {len(df.columns)} columns")
        return df
    ...
    def preprocess_news_data(self, news_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Remove duplicates
        duplicate_cols = ['title', 'symbol'] if 'title' in df.columns and 'symbol' in df.columns else None
        if duplicate_cols:
            df = df.drop_duplicates(subset=duplicate_cols, keep='first')
        
        self.logger.info(f"Processed news data: {len(df)} rows, {len(df.columns)} columns")
        return df
    ...
    def preprocess_sec_data(self, sec_df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.logger.info(f"Processed SEC data: {len(df)} rows, {len(df.columns)} columns")
        return df
    ...
    def _add_market_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'MA_50' in df.columns:
            market_trend = df.groupby('Date').apply(
                lambda x: (x['Close'] > x['MA_50']).mean()
            ).reset_index()
    ...
    def create_risk_labels(self, df: pd.DataFrame, volatility_threshold: float = 0.3,
                          return_threshold: float = -0.1, window: int = 5) -> pd.DataFrame:
        """
        ...
        self.logger.info(f"Created risk labels: {df['Risk_Label'].sum()}/{len(df)} high-risk periods")
        return df
