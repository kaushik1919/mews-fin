    ...
    def fetch_yahoo_data(
        ...
        for symbol in symbols:
            try:
                ...
                hist_data["Volatility_30d"] = hist_data["Returns"].rolling(
                    window=30
                ).std() * np.sqrt(252)
            ...
            for symbol in symbols:
                if symbol in stock_data:
                    ...
                    if fundamentals:
                        ...
                        df["Gross_Profit_TTM"] = fundamentals.get(
                            "GrossProfitTTM", np.nan
                        )
                        df["EBITDA"] = fundamentals.get("EBITDA", np.nan)
                        df["Operating_Margin_TTM"] = fundamentals.get(
                            "OperatingMarginTTM", np.nan
                        )
