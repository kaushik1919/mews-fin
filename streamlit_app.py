    ...
    for symbol in symbols:
        try:
            ...
            if response.status_code == 200:
                ...
                st.error(f"API Error for {symbol}: {response.status_code} - {response.text[:200]}")
    ...
    try:
        ...
        if stock_data is not None and 'Risk_Label' in stock_data.columns:
            if 'Date' in stock_data.columns:
                ...
                daily_risk = stock_data.groupby('Date')['Risk_Label'].mean().reset_index()
        ...
        fig.update_layout(
            title="Combined Sentiment & Risk Analysis",
            height=600,
            showlegend=True
        )
...
def create_model_comparison_chart(results):
    ...
    fig.add_trace(
        go.Bar(x=models, y=auc_scores, name='AUC Score', 
               marker_color='lightcoral', text=[f'{auc:.3f}' for auc in auc_scores],
               textposition='auto'),
        row=1, col=2
    )
...
def create_feature_importance_chart(results):
    ...
    colors = ['rgba(255,99,71,0.8)' if score > 0.1 else 
              'rgba(255,165,0,0.8)' if score > 0.05 else 
              'rgba(135,206,235,0.8)' for score in scores]
...
def create_risk_timeline(df):
    ...
    try:
        ...
        daily_risk = timeline_data.groupby('Date').agg({
            'Risk_Label': ['mean', 'std', 'count'],
            'Symbol': 'nunique'
        }).round(3)
        
        daily_risk.columns = ['Risk_Level', 'Risk_Volatility', 'Sample_Count', 'Symbols_Count']
        ...
        fig.add_hrect(y0=0.7, y1=1, fillcolor="red", opacity=0.1,
...
def main():
    ...
    # Header
    st.markdown('<h1 class="main-header">📊 Market Risk Early Warning System</h1>', unsafe_allow_html=True)
    ...
    if df is None or results is None:
        ...
        st.error("❌ No data available. Please run the analysis first using the command line tool.")
    ...
    st.sidebar.info(f"Symbols: {df['Symbol'].nunique() if 'Symbol' in df.columns else 'N/A'}")
    ...
    with tab1:
        ...
        for model_name, metrics in results.items():
            if model_name not in ['test_data', 'ensemble'] and isinstance(metrics, dict):
                model_count += 1
    ...
    with tab2:
        ...
        st.info("💡 This shows which market factors are most important for predicting risk. Think of it like a recipe - these are the main ingredients that create market risk!")
        ...
        if importance_fig:
            ...
            st.warning("Feature importance data not available - please run the models first")
        
        # Feature correlation heatmap
        if len(df.select_dtypes(include=[np.number]).columns) > 1:
            st.subheader("🔗 How Market Factors Relate to Each Other")
            st.info("💡 This heatmap shows which factors move together. Dark red = they move in the same direction, dark blue = they move opposite to each other.")
            
            numeric_cols = df.select_dtypes(include=[np.number]).columns[:15]  # Limit for performance
            ...
            fig.update_layout(
                title="🔗 Market Factor Relationships",
                height=600
            )
            ...
            with st.expander("❓ How to Read This Chart"):
                ...
                st.write("• **Tip**: Look for clusters of red or blue to find factor groups")
    
    with tab3:
        st.subheader("📊 Market Risk Timeline - Your Risk Radar")
        
        st.info("💡 **What am I looking at?** This shows how risky the market was on different days. Think of it like a weather forecast - but for your investments!")
        ...
        if timeline_fig:
            ...
            st.error("❌ Could not create risk timeline. Please ensure the data has been processed correctly.")
        ...
        if 'Risk_Label' in df.columns:
            ...
            with col1:
                ...
                fig = px.pie(
                    ...
                    names=['Safe Days 🟢' if x == 0 else 'Risky Days 🔴' for x in risk_counts.index],
            ...
            with col2:
                ...
                # Risk insights
                if risky_pct > 30:
                    st.error("⚠️ **High Risk Period**: More than 30% of days were risky!")
                elif risky_pct > 20:
                    st.warning("🔶 **Moderate Risk**: About 1 in 5 days were risky")
                else:
                    st.success("✅ **Low Risk Period**: Most days were safe for investing")
        ...
        if 'Risk_Label' in df.columns and 'Date' in df.columns:
            ...
            with col1:
                st.metric("Last 30 Days Risk", f"{recent_risk:.2f}", 
    ...
    with tab4:
        ...
        gnews_api_key = st.text_input("GNews API Key", type="password", 
                                     help="Get your free API key from https://gnews.io/")
        
        # Symbol selection
        available_symbols = df['Symbol'].unique().tolist() if 'Symbol' in df.columns else ['AAPL', 'MSFT', 'GOOGL']
        selected_symbols = st.multiselect("Select symbols for news analysis", 
                                        available_symbols, 
        ...
        if st.button("🔍 Fetch & Analyze News Sentiment", type="primary"):
            if not gnews_api_key:
                ...
                with st.spinner("Fetching news and analyzing sentiment..."):
                    # Fetch news
                    news_df = fetch_gnews_sentiment(selected_symbols, gnews_api_key, days_back)
                    ...
                    if news_df is not None and not news_df.empty:
                        ...
                        if news_with_sentiment is not None:
                            ...
                            st.success(f"✅ Analyzed {len(news_with_sentiment)} news articles")
                            
                            col1, col2, col3, col4 = st.columns(4)
                            
                            avg_sentiment = news_with_sentiment['sentiment_compound'].mean()
                            positive_pct = (news_with_sentiment['sentiment_compound'] > 0.1).mean() * 100
                            negative_pct = (news_with_sentiment['sentiment_compound'] < -0.1).mean() * 100
                            ...
                            if avg_sentiment > 0.2:
                                st.success("🚀 **Very Positive Sentiment**: News is overwhelmingly good! This often leads to price increases.")
                            elif avg_sentiment > 0.05:
                                st.info("📈 **Positive Sentiment**: More good news than bad. Generally bullish for prices.")
                            elif avg_sentiment < -0.2:
                                st.error("📉 **Very Negative Sentiment**: Lots of bad news. Prices may decline.")
                            elif avg_sentiment < -0.05:
                                st.warning("📊 **Negative Sentiment**: More bad news than good. Be cautious.")
                            else:
                                st.info("⚖️ **Neutral Sentiment**: News is balanced. Market may be stable.")
                            ...
                            sentiment_chart = create_sentiment_risk_chart(news_with_sentiment, df)
                            if sentiment_chart:
                            ...
                            symbol_sentiment = news_with_sentiment.groupby('symbol').agg({
                                'sentiment_compound': ['mean', 'std', 'count'],
                                'sentiment_positive': 'mean',
                                'sentiment_negative': 'mean'
                            }).round(3)
                            
                            symbol_sentiment.columns = ['Avg_Sentiment', 'Volatility', 'Article_Count', 'Positive_Score', 'Negative_Score']
                            
                            # Add interpretation column
                            def interpret_sentiment(score):
                                if score > 0.2: return "Very Positive 🚀"
                                elif score > 0.05: return "Positive 📈"
                                elif score < -0.2: return "Very Negative 📉"
                                elif score < -0.05: return "Negative 📊"
                                else: return "Neutral ⚖️"
                            
                            symbol_sentiment['Interpretation'] = symbol_sentiment['Avg_Sentiment'].apply(interpret_sentiment)
                            ...
                            for symbol in selected_symbols:
                                if symbol in symbol_sentiment.index:
                                    score = symbol_sentiment.loc[symbol, 'Avg_Sentiment']
                                    count = symbol_sentiment.loc[symbol, 'Article_Count']
                                    
                                    if score > 0.1:
                                        st.success(f"📈 **{symbol}**: Positive news trend ({count} articles). Good time to consider buying.")
                                    elif score < -0.1:
                                        st.error(f"📉 **{symbol}**: Negative news trend ({count} articles). Consider waiting or selling.")
                                    else:
                                        st.info(f"⚖️ **{symbol}**: Neutral news ({count} articles). No strong signal either way.")
                            
                            # Recent news headlines with better formatting
                            st.subheader("📰 Recent Headlines & Their Sentiment")
                            
                            for symbol in selected_symbols:
                                symbol_news = news_with_sentiment[news_with_sentiment['symbol'] == symbol].head(3)
                                if not symbol_news.empty:
                                    st.markdown(f"### **{symbol}**")
                                    
                                    for i, (_, article) in enumerate(symbol_news.iterrows(), 1):
                        ...
                        st.warning("No news articles found for the selected symbols and time range")
