...
class MarketRiskSystem:
    ...
    def __init__(self, config_path: str = None):
        ...
        if not config_valid:
            self.logger.warning("Some API keys are missing - functionality will be limited")
    ...
    def run_full_pipeline(self, symbols: List[str] = None, start_date: str = None, 
                         end_date: str = None, skip_data_collection: bool = False) -> Dict:
        """
    ...
    def collect_all_data(self, symbols: List[str], start_date: str, end_date: str):
        ...
        self.sec_data = self.sec_downloader.fetch_filings_for_symbols(
            symbols[:10],  # Limit to avoid rate limits
        ...
        if self.stock_data:
            if isinstance(self.stock_data, dict):
                ...
                if stock_dfs:
                    ...
                    self.stock_data = self.preprocessor.preprocess_stock_data(combined_stock)
                else:
                    self.stock_data = None
            else:
                # Ensure single DataFrame has proper index
                self.stock_data = self.stock_data.reset_index(drop=True)
                self.stock_data = self.preprocessor.preprocess_stock_data(self.stock_data)
    ...
    def analyze_sentiment(self):
        ...
        if self.news_data is not None and not self.news_data.empty:
            self.news_data = self.sentiment_analyzer.analyze_news_sentiment(self.news_data)
        ...
        if not sentiment_aggregated.empty:
            sentiment_aggregated = self.sentiment_analyzer.calculate_sentiment_indicators(
    ...
    def integrate_all_data(self) -> Dict[str, List[str]]:
        ...
        feature_groups = self.data_integrator.create_feature_groups(self.integrated_data)
    ...
    def generate_predictions(self):
        ...
        exclude_cols = ['Date', 'Symbol', 'Risk_Label', 'Risk_Score'] + \
                      [col for col in self.integrated_data.columns if self.integrated_data[col].dtype == 'object']
        
        feature_cols = [col for col in self.integrated_data.columns 
                       if col not in exclude_cols and 
                       self.integrated_data[col].dtype in ['int64', 'float64']]
        
        X = self.integrated_data[feature_cols].fillna(0)
        
        # Generate predictions
        predictions, probabilities = self.ml_models.predict_risk(X, model_type='ensemble')
    ...
    def create_visualizations(self, symbols: List[str]) -> Dict[str, str]:
        ...
        feature_importance = self.ml_models.get_feature_importance('random_forest', top_n=15)
    ...
    def run_backtesting(self) -> Dict:
        ...
        if backtest_results:
            report_path = self.backtester.generate_backtest_report(self.config.OUTPUT_DIR)
    ...
    def generate_final_report(self, pipeline_results: Dict) -> str:
        ...
        **Stages Completed:** {len(pipeline_results.get('stages_completed', []))}
        ...
        if model_perf:
            for model_name, metrics in model_perf.items():
                if isinstance(metrics, dict) and 'auc_score' in metrics:
                    ...
                    report_content += f"Accuracy = {metrics.get('test_accuracy', 0):.3f}\\n"
        ...
        for group, count in pipeline_results.get('feature_groups', {}).items():
            report_content += f"- **{group.replace('_', ' ').title()}:** {count} features\\n"
...
def main():
    ...
    try:
        if args.full_pipeline:
            ...
            if system.integrated_data is not None:
                viz_paths = system.create_visualizations(args.symbols or ['AAPL', 'MSFT', 'GOOGL'])
